...
export default function About() {
  return (
    <section id="about" className="py-12 sm:py-16 md:py-20 lg:py-28 px-4 sm:px-6 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-3 sm:mb-5 leading-tight">
            We started in 2019 with a simple bet  <br className="hidden md:block" />
            <span className="text-dark-400">that staffing could be faster, and software could be better.</span>
          </h2>
          <p className="text-dark-400 leading-relaxed text-sm sm:text-base md:text-[15px] max-w-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 sm:gap-8 md:gap-10 items-center">
          <motion.div
          ...
          >
            <img
              ...
              alt="Team at work"
              className="rounded-lg sm:rounded-xl lg:rounded-2xl w-full object-cover"
          ...
          >
            {numbers.map(({ value, text }) => (
              <div key={value} className="pb-5 sm:pb-7" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <p className="text-white font-display font-bold text-2xl sm:text-3xl md:text-4xl mb-1">{value}</p>
                <p className="text-dark-400 text-xs sm:text-sm md:text-base">{text}</p>
              </div>
            ))}
          </motion.div>
        </div>
