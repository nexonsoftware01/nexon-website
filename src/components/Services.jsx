...
export default function Services() {
  return (
    <section id="services" className="py-20 lg:py-28 px-5 sm:px-8" style={{ background: 'rgba(15,23,42,0.3)' }}>
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-dark-400 text-[15px] max-w-2xl mx-auto">
          ...
          {SERVICES.slice(0, 3).map((service, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {Icon && <Icon className="w-6 h-6" style={{ color: accents[i] }} />}
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {service.features.map((f) => (
                    <span key={f} className="text-xs text-dark-400 px-2.5 py-1 rounded-md" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                      {f}
                    </span>
                  ))}
                </div>
                <a href="#contact" className="inline-flex items-center gap-1.5 text-sm font-medium transition-colors" style={{ color: accents[i] }}>
                  Get started <HiArrowRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-1" />
                </a>
              </motion.div>
            );
          })}
          ...
          {SERVICES.slice(3).map((service, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {Icon && <Icon className="w-6 h-6" style={{ color: accents[realIndex] }} />}
                </div>
                <h3 className="text-white font-display font-semibold text-lg mb-2">{service.title}</h3>
                <p className="text-dark-400 text-sm leading-relaxed mb-5">{service.description}</p>
                <div className="flex flex-wrap gap-2 mb-5">
                ...
                <a href="#contact" className="inline-flex items-center gap-1.5 text-sm font-medium transition-colors" style={{ color: accents[realIndex] }}>
                  Get started <HiArrowRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-1" />
                </a>
              </motion.div>
            );
